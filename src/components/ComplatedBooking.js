...
function PaymentBadge({ status }) {
  if (status === "Pending") {
    return <span style={{ fontSize: 12, color: "#9ca3af", fontStyle: "italic" }}>Pending</span>;
  }
  const map = { Paid: { bg: "#22c55e", color: "#fff" }, Refunded: { bg: "#fef9c3", color: "#a16207" } };
  const s = map[status] || map.Paid;
  return (
    <span style={{ display: "inline-block", fontSize: 12, fontWeight: 600, color: s.color, background: s.bg, padding: "3px 14px", borderRadius: 5 }}>
      {status}
    </span>
  );
}
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", background: "#f7f8fa", padding: isMobile ? "16px 14px" : "26px 30px" }}>
      ...
      <div style={{ fontSize: 12, color: "#9ca3af", marginBottom: 22 }}>
        ...
        {" / "}
        <span onClick={onBack} style={{ color: "#2563eb", cursor: "pointer" }}>Completed Bookings</span>
        {" / "}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {booking.timeline.map((t, i) => (
                <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                  <div style={{ width: 7, height: 7, borderRadius: "50%", marginTop: 5, flexShrink: 0, background: "#22c55e" }} />
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", background: "#f7f8fa", padding: isMobile ? "16px 14px" : "28px 32px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22 }}>
        <h1 style={{ margin: 0, color: "#111", fontSize: isMobile ? 18 : 24 }}>Completed Bookings</h1>
        ...
        {isMobile ? (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {paginated.length === 0 ? (
                <tr>
                  <td colSpan={8} style={{ padding: 36, textAlign: "center", color: "#9ca3af", fontSize: 13 }}>No completed bookings found.</td>
                </tr>
              ) : paginated.map((b, i) => (
                <tr key={i}
